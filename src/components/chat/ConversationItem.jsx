export default function ConversationItem({
  ...
  const otherUser = conversation.participants?.find(u => u._id !== currentUser._id);
  const isOnline = onlineUsers.includes(otherUser?._id);

  const activeClass = active ? "bg-blue-50 border-l-4 border-blue-500" : "hover:bg-gray-50";
  ...
  const getLastMessagePreview = () => {
    ...
    if (lastMessage.text) return lastMessage.text.length > 30 ? lastMessage.text.slice(0, 30) + "..." : lastMessage.text;
    return "Message";
  };
  ...
  const getDisplayName = () => otherUser?.name || otherUser?.username || "User";
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex justify-between items-start">
          ...
          <span className="text-xs text-gray-500 whitespace-nowrap ml-2">{formatTime()}</span>
        </div>
        ...
        <div className="flex items-center gap-1">
          ...
          <p className="text-sm text-gray-500 truncate flex-1">{getLastMessagePreview()}</p>
